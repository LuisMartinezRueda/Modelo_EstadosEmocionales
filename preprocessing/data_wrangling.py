...
def combine_files(subjects):
    ...
    # Asegura que las columnas '0', '1' y '2' existen
    missing_cols = [col for col in ['0', '1', '2'] if col not in df.columns]
    if missing_cols:
        raise ValueError(f"Faltan las columnas: {missing_cols} en los archivos CSV.")

    # Confirmamos que cada fila tiene una sola clase activa
    df['label_sum'] = df[['0', '1', '2']].sum(axis=1)
    if not all(df['label_sum'] == 1):
        raise ValueError("Existen filas con más o menos de una clase activa.")

    df.drop('label_sum', axis=1, inplace=True)

    # Conversión segura de dummies a etiqueta numérica
